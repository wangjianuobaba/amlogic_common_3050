...
static void vdin_put_timer_func(unsigned long arg)
{
    ...
    tvin_check_notifier_call(TVIN_EVENT_INFO_CHECK, (void*)devp);
...
#if defined(CONFIG_ARCH_MESON)|| defined (CONFIG_ARCH_MESON3)|| defined(CONFIG_ARCH_MESON6)
    switch (port>>8)
    {
        case 0x01: // mpeg
        case 0x02: // 656
...
static int vdin_probe(struct platform_device *pdev)
{
    ...
    for (i = 0; i < VDIN_COUNT; ++i)
    {
        ...
        vdin_devp[i]->dec_lock = __SPIN_LOCK_UNLOCKED(vdin_devp[i]->dec_lock);
        vdin_devp[i]->dec_ops = NULL;

        vdin_devp[i]->priv_data = NULL;
