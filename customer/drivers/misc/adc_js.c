...
static int key_flag_num = sizeof(gamekeys)/sizeof(gamekeys[0]) - 2;
static unsigned int keyvalue = 0;

static void gpio_init(void)
{
	gpio_set_status(PAD_GPIOAO_6,gpio_status_in);
	gpio_set_status(PAD_GPIOAO_7,gpio_status_in);
	gpio_set_status(PAD_GPIOAO_8,gpio_status_in);
	gpio_set_status(PAD_GPIOAO_9,gpio_status_in);
	gpio_set_status(PAD_GPIOE_10,gpio_status_in);
	gpio_set_status(PAD_GPIOX_0,gpio_status_in);
	gpio_set_status(PAD_GPIOX_1,gpio_status_in);
	gpio_set_status(PAD_GPIOX_8,gpio_status_in);
	gpio_set_status(PAD_GPIOX_9,gpio_status_in);
	gpio_set_status(PAD_GPIOX_10,gpio_status_in);
	gpio_set_status(PAD_GPIOX_23,gpio_status_in);
	gpio_set_status(PAD_GPIOX_24,gpio_status_in);
}

static void read_keys_value(void)
{
	gamekeys[0].value = gpio_in_get(PAD_GPIOAO_6);
	gamekeys[1].value = gpio_in_get(PAD_GPIOAO_7);
	gamekeys[2].value = gpio_in_get(PAD_GPIOAO_8);
	gamekeys[3].value = gpio_in_get(PAD_GPIOAO_9);
	gamekeys[4].value = gpio_in_get(PAD_GPIOX_10);
	gamekeys[5].value = gpio_in_get(PAD_GPIOE_10);
	gamekeys[6].value = 0;
	gamekeys[7].value = 0;
	gamekeys[8].value = gpio_in_get(PAD_GPIOX_9);
	gamekeys[9].value = gpio_in_get(PAD_GPIOX_8);
	gamekeys[10].value = gpio_in_get(PAD_GPIOX_0);
	gamekeys[11].value = gpio_in_get(PAD_GPIOX_1);
	gamekeys[12].value = gpio_in_get(PAD_GPIOX_24);
	gamekeys[13].value = gpio_in_get(PAD_GPIOX_23);
}
	...
	if ((key_param[0] == 1 || key_param[0] == 2) && key_param[1] >= 0 && key_param[2] >= 0) {
		...
		if ((kp->key_valid[2] == 1) || (kp->key_valid[3] == 1)) {
			...
			if(second0 < CENTER_TRY) {
				...
				keytouch_report(kp, key_param[1] +  (kp->key_value[2] - 512) * key_param[3] / 512, 
	...
	if (key_param[0] == 1 && key_param[21] >= 0 && key_param[22] >= 0) { //mode 1
		...
		if ((kp->key_valid[0] == 1) || (kp->key_valid[1] == 1)) {
			...
			if(second1 < CENTER_TRY) {
				...
				keytouch_report(kp, key_param[20] +  (kp->key_value[1] - 512) * key_param[22] / 512, 
						key_param[21] +  (512 - kp->key_value[0]) * key_param[22] / 512, 1);
			}
		} else if (kp->circle_flag[1] == 1) {
			kp->circle_flag[1] = 0;
			second1 = 0;
