...
static DEVICE_ATTR(enable,  S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, mc32x0_enable_show, mc32x0_enable_store);
...
static long mc32x0_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			printk("fwq GSENSOR_IOCTL_READ_RAW_DATA\n");
		...
		case GSENSOR_MCUBE_IOCTL_SET_CALI:
			printk("fwq GSENSOR_MCUBE_IOCTL_SET_CALI!!\n");
		...
		case GSENSOR_IOCTL_SET_CALI_MODE:
			printk("fwq mc32x0 GSENSOR_IOCTL_SET_CALI_MODE\n");
			break;
		case GSENSOR_MCUBE_IOCTL_READ_RBM_DATA:
			printk("fwq GSENSOR_MCUBE_IOCTL_READ_RBM_DATA\n");
			data1 = (void __user *) arg;
			if(data1 == NULL)
			{
				ret = -EINVAL;
		...
		case GSENSOR_MCUBE_IOCTL_CLEAR_RBM_MODE:
			printk("fwq GSENSOR_MCUBE_IOCTL_CLEAR_RBM_MODE\n");
			MC32X0_rbm(client, 0);
			break;
		case GSENSOR_MCUBE_IOCTL_REGISTER_MAP:
			printk("fwq GSENSOR_MCUBE_IOCTL_REGISTER_MAP\n");
			break;
#endif
		default:
			ret = -EINVAL;
		...
		const struct i2c_device_id *id)
{
	...
	ret = i2c_smbus_read_byte_data(client, 0x18);
	if (ret != 0x1) {
		printk("------------- cannot find mc32x0 IC -----------------\n");
		return -1;
	}
...
static int __devinit mc32x0_remove(struct i2c_client *client)
{
