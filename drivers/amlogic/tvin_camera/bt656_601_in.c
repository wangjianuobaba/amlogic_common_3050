...
#if defined(CONFIG_ARCH_MESON6)
    WRITE_CBUS_REG(BT_VIDEOSTART, 3 | (3 << 16));   //Line number of the first video start line in field 0/1.there is a blank
    WRITE_CBUS_REG(BT_VIDEOEND , (am656in_dec_info.active_line+3)|          //  Line number of the last video line in field 1. added video end for avoid overflow.
                                    ((am656in_dec_info.active_line+3) << 16));                   // Line number of the last video line in field 0
#else
...
{
    //unsigned status   = READ_CBUS_REG(BT_STATUS);
    //pr_info("+++bt656 status 0x%x\n",status);
}
#endif
...
*/
int amvdec_656_601_camera_in_run(vdin_dev_t* devp,vframe_t *info)
{
    unsigned ccir656_status;
