...
//hycui====================
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2640
static int ov2640_have_inited = 0;
#endif
...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc = 66,     ////150,
    .pmu_battery_cap = 3760,//3000,    ////8000,
...
static int gc0308_v4l2_uninit(void)
{
    ...
    gpio_out(PAD_GPIOE_11, 1);    // set camera power disable
    msleep(5);
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2640)
    if (ov2640_have_inited == 0)
#endif
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_G30S10A
#include <linux/ft5x06_s10a.h>
#define GPIO_FT_RST  PAD_GPIOC_3
#define GPIO_FT_IRQ  PAD_GPIOA_16
#define GPIO_FT_WAKEUP PAD_GPIOA_3
...
#if CONFIG_EKTF2132HW_CAPACITIVE_TOUCHSCREEN_G13_SP0738J
...
static struct elan_ktf2k_i2c_platform_data ts_pdata = {
/*	.irq = FT_IRQ,
	.init_irq = ts_init_irq,
	.power = ts_power,
	.screen_max_x=1024,
...
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2640)
static pinmux_item_t ov2640_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t ov2640_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &ov2640_pins[0]
};

static int ov2640_init(void)
{
	    gpio_out(PAD_GPIOE_11, 1);    // set camera power enable
		pinmux_set(&ov2640_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock			
    ...
    printk("ov2640_init OK!!!!\n");
    return 0;
}


static int ov2640_v4l2_init(void)
{
    ov2640_have_inited=1;
		ov2640_init();
}

static int ov2640_v4l2_uninit(void)
{
   ov2640_have_inited=0;
    gpio_out(PAD_GPIOE_10, 1);    // set camera power disable
    msleep(5);
    #if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0308)
    if(gc0308_have_inited==0)
...
static void ov2640_v4l2_early_suspend(void)
{
}

static void ov2640_v4l2_late_resume(void)
{
	  pinmux_set(&ov2640_pinmux_set);
...
aml_plat_cam_data_t video_ov2640_data = {
	.name="video-ov2640",
	.video_nr=0,
	.device_init=ov2640_v4l2_init,
	.device_uninit=ov2640_v4l2_uninit,
	.early_suspend=ov2640_v4l2_early_suspend,
	.late_resume=ov2640_v4l2_late_resume,
	.device_probe=ov2640_init,
};
#endif
//hycui===============
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2640
	{
		I2C_BOARD_INFO("ov2640_i2c", 0x60 >> 1),
		.platform_data = (void *)&video_ov2640_data,
	},
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV5642
    {
        I2C_BOARD_INFO("ov5642_i2c", 0x30),//0x78 >> 1),
...
#endif
#ifdef CONFIG_EKTF2132HW_CAPACITIVE_TOUCHSCREEN_G13_SP0738J
  {
        I2C_BOARD_INFO("EKTF-2132hw", 0x15),
         .irq = INT_GPIO_0,
...
*BT_EN    GPIOC_7
*WIFI_PWREN		-->GPIOAO_3 //HYCUI   GPIOC_8
*BT_RST		-->GPIOC_9
*WIFI_WAKE		-->GPIOX_11
*BT_WAKE    GPIOX_10
...
static pinmux_item_t audio_i2s_pinmux[] = {
...
static pinmux_set_t audio_i2s_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &audio_i2s_pinmux[0],
};

static void audio_device_init(void)
{
    /* audio pinmux */
    pinmux_set(&audio_i2s_pinmux_set);

    /* GPIOA_19 PULL_UP_REG0 bit19 */
    aml_set_reg32_bits(P_PAD_PULL_UP_REG0, 1, 19, 1);
}

static void audio_device_deinit(void)
{
    pinmux_clr(&audio_i2s_pinmux_set);
}

 
#if defined(CONFIG_SND_SOC_RT5633)
static int rt5633_hp_detect(void)
{
#define HP_PLUG     1
#define HP_UNPLUG   0
    ...
    val = aml_get_reg32_bits(P_PREG_PAD_GPIO0_I, 19, 1);  // value

    return val;
}
 
static struct rt5633_platform_data rt5633_pdata = {
    .hp_detect      = rt5633_hp_detect,
    .device_init    = audio_device_init,
    .device_uninit  = audio_device_deinit,
};

static struct platform_device aml_rt5633_audio = {
    .name           = "aml_rt5633_audio",
   .id             = 0,
   .resource       = aml_m6_audio_resource,
    .num_resources  = ARRAY_SIZE(aml_m6_audio_resource),
    .dev = {
       .platform_data = &rt5633_pdata,
    },
};
#endif
