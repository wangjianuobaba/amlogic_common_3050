...
extern int am_demod_i2c_xfer(struct aml_demod_i2c *adap, struct i2c_msg *msgs, int num);
...
repeat:
	i2c_flag = am_demod_i2c_xfer(si2176, msg, 1);
    if (i2c_flag < 0) {
        ...
        si2176_cmdreplyobj_t *rsp)
{
    ...
    if (si2176_writecommandbytes(si2176, 8, cmdbytebuffer) != 8)
    {
        error_code = ERROR_SI2176_SENDING_COMMAND;
        //pr_info("%s: write command byte error:%d!!!!\n", __func__, error_code);
        printk("%s: write command byte error:%d!!!!\n", __func__, error_code);
    }
    ...
    if (!error_code)
    {
        ...
        if (error_code)
            //pr_info("%s: poll response error:%d!!!!\n", __func__, error_code);
            printk("%s: poll response error:%d!!!!\n", __func__, error_code);
...
************************************************************************************************************************/
static int si2176_setupdtvdefaults(si2176_propobj_t *prop, struct aml_tuner_sys * tuner_sys)
{
	...
	prop->dtv_mode.modulation = tuner_sys->mode;
	printk("tuner amp is %d\n",tuner_sys->mode);
	...
	prop->dtv_lif_out.amp=25;
	prop->dtv_lif_out.amp=tuner_sys->amp;
	printk("tuner amp is %d\n",tuner_sys->amp);
	
 /*   if(ch_mode == 4)
        prop->dtv_lif_out.amp=23;//27;
    else
    ...
    prop->dtv_int_sense.chlnegen=SI2176_DTV_INT_SENSE_PROP_CHLPOSEN_ENABLE;
...
static int si2176_setupcommondefaults(si2176_propobj_t *prop)
{
	...
	prop->master_ien.atvien = SI2176_MASTER_IEN_PROP_ATVIEN_OFF;
	prop->master_ien.ctsien = SI2176_MASTER_IEN_PROP_CTSIEN_ON;
	prop->master_ien.dtvien = SI2176_MASTER_IEN_PROP_DTVIEN_ON;//SI2176_MASTER_IEN_PROP_DTVIEN_ON;
	prop->master_ien.errien = SI2176_MASTER_IEN_PROP_ERRIEN_ON;//SI2176_MASTER_IEN_PROP_ERRIEN_ON;
...
int si2176_tune(struct i2c_client *si2176, unsigned char mode, unsigned long freq, si2176_cmdreplyobj_t *rsp, si2176_common_reply_struct *common_reply)
{
    ...
    if (si2176_tuner_tune_freq(si2176, mode, freq, rsp) != 0)
	{
        ...
        printk("%s: tuner tune freq error:%d!!!!\n", __func__, ERROR_SI2176_SENDING_COMMAND);
        return ERROR_SI2176_SENDING_COMMAND;
    }
#if 0
    ...
    if (!count)
    {
        ...
        printk("%s: ERROR_SI2176_TUNINT_TIMEOUT error:%d!!!!\n", __func__, ERROR_SI2176_TUNINT_TIMEOUT);
        return ERROR_SI2176_TUNINT_TIMEOUT;
    }
    else
        printk("count is %d \n", 75-count);
    /* wait for xTVINT, timeout is 350ms for ATVINT and 6 ms for DTVINT */
    //count1 = ((mode==SI2176_TUNER_TUNE_FREQ_CMD_MODE_ATV) ? 300 : 3);
    count1 = ((mode==SI2176_TUNER_TUNE_FREQ_CMD_MODE_ATV) ? 300 : 3);
    for (;count1 ;count1--)
	{
        if ((return_code = si2176_check_status(si2176, common_reply)) != 0)
    ...
    if (!count1)
    {
        ...
        printk("%s: ERROR_SI2176_XTVINT_TIMEOUT error:%d!!!!\n", __func__, ERROR_SI2176_XTVINT_TIMEOUT);
        return ERROR_SI2176_XTVINT_TIMEOUT;
    }
    printk("count is %d, count1 is %d \n", 75-count, 75-count1);
#endif
    return return_code;
}
...
*/
	...
	si2176_setupdtvdefaults(prop, tuner_sys);
...
int si2176_powerupwithpatch(struct i2c_client *si2176, si2176_cmdreplyobj_t *rsp, si2176_common_reply_struct *common_reply)
{
    ...
    return_code = si2176_power_up(si2176,          /* always wait for CTS prior to POWER_UP command */
                                  ...
                                  SI2176_POWER_UP_CMD_FUNC_NORMAL,//SI2176_POWER_UP_CMD_FUNC_BOOTLOADER,        /* start in bootloader mode */
